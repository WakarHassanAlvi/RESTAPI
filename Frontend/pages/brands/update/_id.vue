<template>
  <div style="padding: 20px 70px; text-align: center;">
    <h1 style="text-align: center; margin-bottom: 20px;">Update Brand</h1>
...
export default {
  ...
  async mounted() {
    try {
      let { data } = await this.$axios.get(`/brands/${this.$route.params.id}`);

      this.form.name = data.name;
      this.form.url = data.url;
      this.form.active = data.active;
      this.form.date = data.date;
      this.form.operator_id = data.operator_id;
    } catch(err) {
      console.log(err);
    }
  },
  methods: {
    async onSubmit(evt) {
      try {
        evt.preventDefault();
        let { data } = await this.$axios.put(`/brands/${this.$route.params.id}`, this.form);
